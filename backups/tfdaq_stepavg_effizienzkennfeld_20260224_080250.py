...
def plot_efficiency_map(data, savepath='efficiency_map.png', save_svg=False, highlight_global_max=False):
  try:
    if data is None:
    ...
    if span_n >= 3000:
      ...
      n_step = 100.0
    n_group = np.round(n_abs / n_step) * n_step
    finite_n = np.isfinite(n_group)
    unique_groups = np.sort(np.unique(n_group[finite_n])) if np.any(finite_n) else np.array([])
    ...
    for rpm in unique_groups:
      idx = np.where(n_group == rpm)[0]
      if len(idx) < 4:
        continue
      xg = x[idx]
      zg = z[idx]
    ...
    if cs is not None:
      # Stable fallback labeling (avoids backend/version-specific contour internals).
      labels = ax.clabel(cs, levels_line[::2], inline = True, fmt = '%.2f', fontsize = 9)
      for t in labels:
    ...
    # Use precomputed speed groups for speed-line overlay.
    if n_groups <= 20:
      line_colors = plt.cm.tab20(np.linspace(0.0, 1.0, max(1, n_groups)))
    else:
      # For many speed lines: generate many unique hues and darken slightly.
      line_colors = plt.cm.hsv(np.linspace(0.02, 0.98, max(1, n_groups), endpoint=False))
    ...
    for gi, rpm in enumerate(unique_groups):
      ...
      style = line_styles[gi % len(line_styles)]
      ...
      (h,) = ax.plot(
        x_plot, y_plot, style, color = color, linewidth = 1.9,
        alpha = 0.98, zorder = 8
      )
      handles.append(h)
    ...
    plt.close(fig)
    print(f'  efficiency map saved: {png_file} / {pdf_file} ({method_txt})')
...
def plot_motor_efficiency_map_torque_speed(df, out_dir = ".", fname = "motor_eff_map_torque_speed_like_ref"):
  try:
    def _get_col(d, col):
    ...
    plt.close(fig)
    print(f'  motor efficiency map saved: {png_path} / {pdf_path}')
...
def main():
  ...
  try:
    # Get data_files path or filename from user
    inputfiles = input(f'Input data file or path (to process all .csv files): [{default_data_path}] ')
    # Get list of all .csv files if only path is given
    if not inputfiles:
      inputfiles = default_data_path
    if os.path.isdir(inputfiles):
      # Ask user whether path should be searched recursively
      if path_recursive:
      ...
      if path_recursive:
        data_files = [str(f) for f in Path(inputfiles).rglob('*.csv') if (f.is_file() and ('_avg.csv' not in str(f)))]
      else:
        data_files = [str(f) for f in Path(inputfiles).glob('*.csv') if (f.is_file() and ('_avg.csv' not in str(f)))]
    ...
    try:
      ...
      plot_efficiency_map(df, savepath = os.path.join(data_file_path, 'efficiency_map.png'))
